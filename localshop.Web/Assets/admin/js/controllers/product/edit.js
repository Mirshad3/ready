$(document).ready(function () {
                ...
                if (response.success) {
                    ...
                    if (listImages.indexOf(newUrl) < 0) {
                        ...
                        $('#productImages').append(`
                            <div class="position-relative m-2">
                                <img src="${newUrl}" />
                                <button class="position-absolute px-1 bg-light clear-image" onclick="deleteImage('${newUrl}')">Delete</button>
                            </div>`);
                    }
                } else {
    ...
    function deleteImage(imageUrl) {
        $.ajax({
            url: '/File/Delete', // Update with your controller action for file deletion
            type: 'POST',
            data: { fileName: imageUrl },
            success: function (response) {
                if (response.success) {
                    // Handle success: update UI or refresh image list
                    console.log('File deleted successfully:', response.message);

                    // For example, you might want to refresh the list of images
                    //refreshImageList();
                } else {
                    // Handle failure: display an error message or take appropriate action
                    console.error('File deletion failed:', response.message);
                }
            },
            error: function (error) {
                // Handle the error response if needed
